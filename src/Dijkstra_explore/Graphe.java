...
public class Graphe {
     ...
     * @param i : abscisse courante
     * @param j : ordonnee courante
     * @param etiquette : indice du noeud dans arcs
     * @param direction : haut, bas, droite ou gauche
     */
    private void lookAround(Plateau p, int i, int j, int etiquette, String direction){
       	...
       	switch(direction){
       		case "haut": var_dir_y = -1; break;
       		case "bas": var_dir_y = 1; break;
       		case "gauche": var_dir_x = -1; break;
       		case "droite": var_dir_x = 1; break;
       		default: 
		...
		if(0 <= i+var_dir_x && i+var_dir_x < 16 && 0 <= j+var_dir_y && j+var_dir_y < 16){ 
			...
			// on affecte a y_in_arcs le bon indice d etiquette en fonction de la direction
	       	switch(direction){
       		case "haut": y_in_arcs = etiquette-16; break;
       		case "bas": y_in_arcs = etiquette+16; break;
       		case "gauche": y_in_arcs = etiquette-1; break;
       		case "droite": y_in_arcs = etiquette+1; break;
       		default:
       			System.out.println("Erreur: mauvaise direction.");
       			System.exit(-1);
       		}
	       	...
	       	switch(direction){
       		case "haut": y_in_p = 15; y_in_arcs = (NCases-1) - (15 - etiquette); break;
       		case "bas": y_in_p = 0;  y_in_arcs = etiquette - (NCases-1 -15); break;
       		case "gauche": x_in_p = 15; y_in_arcs = etiquette + 15; break;
       		case "droite": x_in_p = 0; y_in_arcs = etiquette - 15; break;
       		default:
       			System.out.println("Erreur: mauvaise direction.");
       			System.exit(-1);
       		}
		...
		if((indice > 10 || indice == 0)){ 
			// on cree un arc entre le noeud courant et le noeud a la direction voulue
